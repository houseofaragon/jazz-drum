...
function init() {
    ...
    if ('key' in params) {
		...
		if ('trid' in params && useParamTRID) {
	        ...
	        alert(params['trid']);
			trackID = params['trid'];
			$("ul.options option[value='0']").attr("selected","selected");
...
function initBeatSelector() {
    for (var i = 0; i < drumInfos.length; i++) {
        ...
        opt.attr("data-value", i.toString());
        opt.text(di.name);
        $("ul.options").append(opt);
        //alert($("#select-drummer").html(opt));
    }
    $("#select-drummer").change(
        function() {
            var val = $("#select-drummer option:selected").attr("data-value");
